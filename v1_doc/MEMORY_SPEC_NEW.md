...
- the boundary between memory, research history, and compiled knowledge
...
- raw source custody semantics
- interface display contracts
- telemetry schemas
- test matrices
- roadmap sequencing
- low-level persistence placement
...
It is not a backup truth layer, a note bucket, a research archive, or a rival authority for state, transition, or governance.
...
- research archive and memory collapsing into one bucket
- compiled knowledge and memory collapsing into one bucket
...
Research is for bounded inquiry and source-aware support.  
Compiled knowledge is for durable thematic support organization.  
...
Execution, outcome, evaluation, research, planning, compiled knowledge, and operator input may all produce signals that inform memory work.
...
- a raw source store
- a research history archive
- a compiled-knowledge wiki
- a generic artifact dump

If something belongs in canonical state, policy, research history, compiled knowledge, or another truth-owning or support-owning object family, memory must not impersonate it.

# Boundary With Research History and Compiled Knowledge

Jeff keeps three distinct durable support classes:

1. **Research history / research artifacts**  
   Source-aware direct outputs and dated historical support objects produced by research.

2. **Compiled knowledge**  
   Durable thematic support artifacts built across sources and time.

3. **Memory**  
   Distilled durable continuity likely to matter again.

These classes must not collapse into one blob.

## Research history / research artifact ownership

The following support-object families belong to Research, not Memory:
- `research_brief`
- `research_comparison`
- `evidence_bundle`
- `source_set`
...
They may later support memory work.
They do not become memory automatically.

## Compiled knowledge ownership

The following support-object families belong to Compiled Knowledge, not Memory:
- `source_digest`
- `topic_note`
- `concept_note`
- `comparison_note`
- `contradiction_note`
- `open_questions_note`
- any later bounded topic-map style support artifact canonized by the knowledge layer

These objects are:
- broader than memory
- thematic rather than merely episodic
- navigational and browsable support objects
- not current truth
- not memory by default

They may suggest what seems reusable.
They do not create or commit memory.
...
- memory retrieval
- memory linking and lineage

The correct directional relationship is:

`raw sources -> research history / compiled knowledge -> Memory candidate consideration -> write / reject / defer`

Automatic dumping is forbidden.
...
- a whole research brief is not episodic memory
- a topic note is not semantic memory merely because it contains conclusions
...
- typed links to related project, work unit, run, research artifacts, compiled-knowledge artifacts, source refs, and related memory where relevant
...
- the record must not be a raw dump of logs, traces, prompts, research bodies, history records, or compiled-knowledge notes
...
- no candidate is created merely because a run ended, an artifact exists, a knowledge note exists, or a conversation happened
...
- it is actually a research artifact, history record, or compiled-knowledge note wearing a memory costume
...
- memory is not a research history archive
- memory is not a compiled-knowledge layer
...
- research archive and memory collapsing into one store
- compiled knowledge and memory collapsing into one store
...
- no direct promotion of research artifacts or knowledge artifacts into memory by convenience
...
- turning history records or topic notes into memory just because they are useful
...
- richer compatibility with recurring research-history workflows
- stronger interoperability with compiled-knowledge retrieval and maintenance
...
No unresolved memory-model questions were settled inside this document beyond the boundaries explicitly frozen here.
...
- `PLANNING_AND_RESEARCH_SPEC.md` owns research semantics, research artifacts, evidence, and planning; this document keeps memory subordinate to those research support objects rather than absorbing them.
- `VISION.md` owns the product-level requirement that memory provide continuity without becoming fake truth.

# Relationship to Implementation Architecture Docs

The following implementation documents must remain subordinate to this canonical law:
- raw source custody architecture
- research history / archive architecture
- compiled-knowledge architecture
- memory implementation architecture

Those documents may define storage placement, module layout, retrieval mechanics, maintenance jobs, or persistence detail.
They may not weaken:
- Memory-only candidate creation
- committed-memory-only canonical linking
- truth-first retrieval
- project isolation
- memory vs research-history separation
- memory vs compiled-knowledge separation
...
Research history remains source-backed support.  
Compiled knowledge remains thematic support.  
Memory remains distilled continuity.

If these laws stay hard, Jeff can preserve continuity for both Jeff-project and non-Jeff project work without turning memory into sludge, archive sprawl, or fake truth.
