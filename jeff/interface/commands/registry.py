...
from .scope import json_command, mode_command, project_command, run_command, scope_command, work_command
from .selection import selection_command
...
def execute_command(
    *,
    ...
    commands_surface = importlib.import_module("jeff.interface.commands")
    if not tokens:
    ...
    if tokens[0] == "inspect":
        result = inspect_command(
            ...
            session=session,
            context=context,
            assemble_live_context_package_fn=commands_surface.assemble_live_context_package,
        )
        return _apply_json_mode(result, json_output=json_output)
    if tokens[0] == "show":
