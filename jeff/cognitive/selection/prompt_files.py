"""File-backed prompt loader and renderer for Selection prompt contracts."""
...
_PROMPTS_ROOT = Path(__file__).parents[3] / "PROMPTS" / "selection"
...
class PromptFileNotFoundError(FileNotFoundError):
    """Raised when a required Selection prompt file is not found."""


class PromptFileMalformedError(ValueError):
    """Raised when a Selection prompt file is missing required sections."""


class PromptRenderError(ValueError):
    """Raised when a rendered Selection prompt still has unresolved placeholders."""


def load_prompt_file(file_name: str) -> tuple[str, str]:
    """Load a Selection prompt file and return (system_instructions, prompt_template)."""
...
def render_prompt(template: str, **values: str) -> str:
    """Substitute {{KEY}} placeholders in a Selection prompt template."""
    result = template
